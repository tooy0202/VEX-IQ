...
Fbeam, start_stand_off_up, stakeONGo, MakeY, crash;
...
void Drop_down()
{
  ...
  MotorPin.setVelocity(80, percent);
  stakeONGo ? MotorPin.spinFor(forward, 25.0, degrees, false) : MotorPin.spinFor(forward, 155.0, degrees);
...
void Drop_down_Grab_Up()
{
  ...
  // MotorLeft.setStopping(coast);
  // MotorRight.setStopping(coast);
  if (MotorLeft.velocity(vex::velocityUnits::pct) > 15)
  {
    MotorLeft.spin(forward);
    MotorRight.spin(forward);
    for (float speedMotorPin = MotorLeft.velocity(vex::velocityUnits::pct); speedMotorPin > 1; speedMotorPin -= 5)
    {
      MotorLeft.setVelocity(speedMotorPin, percent);
      MotorRight.setVelocity(speedMotorPin, percent);
      MotorLeft.spin(forward);
      MotorRight.spin(forward);
      wait(10, msec);
    }
    ...
    //   Pneumatic_Pin_Beam.retract(cylinder1);
    //   MotorPin.spinFor(forward, 80.0, degrees, false);
    //   MotorLeft.setVelocity(50.0, percent);
    //   MotorRight.setVelocity(50.0, percent);
    //   MotorLeft.spinFor(forward, 140.0, degrees, false);
    //   MotorRight.spinFor(forward, 140.0, degrees, false);
    //   wait(0.3, seconds);
    //   Pneumatic_Pin_Beam.extend(cylinder1);
...
void ControllerDChanged()
{
  if (Controller.AxisD.position() >= 90.0)
  {
    ...
    guide.retract(cylinder2);
    // wait(300, msec);
    // MotorPin.spinFor(forward, 60.0, degrees);
    // MotorPin.setVelocity(100, percent);
    // stakeONGo = true;
  }
  else if (Controller.AxisD.position() <= -90.0)
  {
    // MotorPin.spinFor(reverse, 60.0, degrees, false);
    // wait(250, msec);
    guide.extend(cylinder2);
    // stakeONGo = false;
  }
