...
void Drop_down_Grab_Up()
{
    ...
    if (robot.Go == true)
    {
        ...
        {
            MotorPin.spinToPosition(145.0, degrees, false);
            Rdown_on = true;
        }
    }
...
void Ajpin()
{
    ...
    MotorLeft.spinFor(forward, 183.5, degrees, false);
    MotorRight.spinFor(forward, 183.5, degrees, true);
    MotorLeft.setStopping(hold);
    MotorRight.setStopping(hold);
    MotorLeft.stop();
    MotorRight.stop();
...
void Drive()
{
    ...
    double turnSpeed = (Controller.AxisB.position() * Controller.AxisB.position()) * (Controller.AxisB.position() * 0.0001) * 0.75;
    ...
    MotorRight.setVelocity((forwardSpeed - turnSpeed) * 1.0, percent);
    MotorLeft.setVelocity((forwardSpeed + turnSpeed) * 1.0, percent);
...
void EUp_pressed()
{
    if (robot.isBusy == false && (controllerState.Rup == ButtonStage::STEP1 || controllerState.Rup == ButtonStage::STEP2))
    {
        ...
        MotorBeam.stop(coast);
        wait(0.2, seconds);
        MotorBeam.spinFor(reverse, 280.0, degrees, false);
        wait(0.7, seconds);
        // robot.Maxturn = 40.0;
        // ControTurn = 0.40;
...
void EDown_pressde()
{
    if (robot.start == true)
    {
        ...
        MotorLeft.spinFor(forward, 0.79, turns, false);
        MotorRight.spinFor(forward, 0.79, turns, true);
        MotorPin.resetPosition();
        MotorPin.setPosition(0.0, degrees);
        MotorPin.setStopping(hold);
        MotorPin.setVelocity(80.0, percent);
        ...
        MotorPin.setMaxTorque(100, pct);
        MotorPin.resetPosition();
        MotorPin.spinToPosition(230.0, degrees, false);
        Extend_joint_pin();
