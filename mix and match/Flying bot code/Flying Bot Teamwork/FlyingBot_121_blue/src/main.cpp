...
void Spin_Robot()
{
    ...
    MotorRight.setStopping(hold);
    MotorLeft.setStopping(hold);
    MotorLeft.spinFor(reverse, 350.0, degrees, false);
    MotorRight.spinFor(forward, 350.0, degrees, true);
    TouchLED.setColor(black);
    robot.isBusy = false;
    // MotorLeft.spin(reverse, 70.0, percent);
    // MotorRight.spin(reverse, 50.0, percent);
    // wait(0.4, seconds);
    for (int speed = 90; speed > 60; speed -= 10)
    {
        MotorLeft.spin(reverse, speed, percent);
        MotorRight.spin(reverse, speed - 20, percent);
        wait(100, msec);
    }
    robot.isBusy = true;
    // wait(0.2, seconds);
...
void ControllerDChanged()
{
    ...
    else if ((controllerState.Rup == ButtonStage::STEP1 || controllerState.Rup == ButtonStage::STEP2) && Controller.AxisD.position() <= -80.0)
    {
        if (robot.isBusy == false)
        {
            robot.stop = true;
